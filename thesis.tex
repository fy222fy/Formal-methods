...
\subsection{逻辑推理}
逻辑推理是基于知识与信念推理的分析方法，主要运用逻辑系统从用户接受和发送的消息出发，通过一系列的推理公推证协议是否满足安全规范\cite{2}。
...
这种方法的不足主要表现在它逻辑的抽象级别较高，分析范围过窄，不能对知识进行推理，只能分析协议的认证性质，只能发现协议的缺陷，不能保证协议一定是安全的\cite{4}。
